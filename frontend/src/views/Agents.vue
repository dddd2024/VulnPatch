<template>
  <div class="agents-page">
    <el-card shadow="hover" class="routing-card">
      <template #header>
        <div class="header-flex"><strong>自主多模型调度决策</strong><el-button size="small" @click="loadRouting">刷新决策</el-button></div>
      </template>
        ...
        <el-table-column type="expand">
          <template #default="{ row }">
            <div class="decision-detail">
              <el-table :data="row.candidates" size="small">
                <el-table-column prop="provider" label="Provider" />
                <el-table-column prop="score" label="Score" />
                <el-table-column prop="health" label="Health" />
                <el-table-column label="Allowed"><template #default="scope">{{ scope.row.allowed }}</template></el-table-column>
                <el-table-column label="Available"><template #default="scope">{{ scope.row.available }}</template></el-table-column>
                <el-table-column label="Reasons"><template #default="scope">{{ scope.row.reasons.join(', ') || '-' }}</template></el-table-column>
              </el-table>
            </div>
          </template>
        </el-table-column>
    ...
    <el-card shadow="hover">
      <template #header>
        <div class="header-flex"><strong>Agent 执行日志</strong><el-button size="small" @click="scanStore.fetchAgentLogs()">刷新</el-button></div>
      </template>
        ...
        <el-table-column type="expand">
          <template #default="{ row }">
            <div style="padding: 12px 24px">
              ...
              <pre v-if="row.metadata && Object.keys(row.metadata).length" class="code-snippet">{{ JSON.stringify(row.metadata, null, 2) }}</pre>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="agent_name" label="Agent" width="160"><template #default="{ row }"><el-tag effect="plain">{{ row.agent_name }}</el-tag></template></el-table-column>
        <el-table-column prop="stage" label="阶段" width="120"><template #default="{ row }"><el-tag size="small" type="info">{{ row.stage }}</el-tag></template></el-table-column>
        <el-table-column prop="message" label="消息" min-width="300" show-overflow-tooltip />
        <el-table-column prop="timestamp" label="时间" width="180"><template #default="{ row }">{{ formatTime(row.timestamp) }}</template></el-table-column>
      </el-table>
...
.decision-detail { padding: 12px 24px; }
.code-snippet { background: #1e1e1e; color: #d4d4d4; padding: 10px 14px; border-radius: 6px; overflow-x: auto; font-size: 12px; line-height: 1.5; }
</style>
