"""
GitLab repository loader for cloning or downloading repositories.

Supports:
- Git clone (requires git installed)
- ZIP download (fallback when git is not available)
- GitLab API v4 (https://gitlab.com/api/v4)
- GitLab Token authentication (environment variable GITLAB_TOKEN)
...
GITLAB_SELF_HOSTED_PATTERN = re.compile(
    r"https?://([^/]+)/([^/]+)/([^/]+?)(?:\.git)?(?:/.*)?"
)
GITLAB_SSH_PATTERN = re.compile(
...
class GitLabLoader:
    ...
    支持通过 git clone 或 ZIP 下载方式获取 GitLab 仓库代码。
    自动管理临时目录并在加载完成后清理。
    ...
    def __init__(
        ...
        Args:
            ...
            gitlab_token: 可选的 GitLab 个人访问令牌。
                未提供时从 ``GITLAB_TOKEN`` 环境变量读取。
            gitlab_url: 可选的自托管 GitLab 实例 URL。
                未提供时默认使用 https://gitlab.com。
        """
    ...
    def load_repo(
        ...
        Raises:
            ValueError: 如果 URL 不是有效的 GitLab URL
            RuntimeError: 如果克隆和下载都失败
        """
        # 解析 GitLab URL
        owner, repo = self._parse_gitlab_url(repo_url)

        logger.info("正在加载 GitLab 仓库: %s/%s (分支: %s)", owner, repo, branch or "默认")
    ...
    def _parse_gitlab_url(self, repo_url: str) -> tuple[str, str]:
        ...
        """
        # 尝试匹配 gitlab.com URL
        match = GITLAB_URL_PATTERN.match(repo_url)
        if match:
            owner = match.group(1)
            repo = match.group(2)
        else:
            ...
            if match:
                ...
                raise ValueError(f"不是有效的 GitLab URL: {repo_url}")
